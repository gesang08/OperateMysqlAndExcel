...
class Package():
    ...
    def Is_NewPackage(self):  # 判断是否需要对组件进行打包算法
        ...
        try:
            ...
            cursor.execute(
                "SELECT `Index` FROM `work_production_scheduling`  WHERE `State`='%s'" % SCHEDULE_LAYOUT_FINISH)
    ...
    def Is_Database_Connected(self):
        try:
            self.db = MySQLdb.connect(host=server_ip, user=user_list[0], passwd=password, db=database[0],
                                      charset=charset)
        except:
    ...
    def Package_CoreProgram(self):
        ...
        try:
            have_board = True  # 是否有门板
            self.part_information = self.door_part_information
            if self.door_part_information==[]:   #无门板
                have_board = False
            for i in range(len(self.part_information)):  # 该循环作用：一个组件门板打几个包
                if (self.part_information != []):
                    ...
                    for j in range(len(self.Box_information)):
                        if 0 < max_door_height <= self.Box_information[j][BOX_LONG] and 0 < max_door_width <= \
                                self.Box_information[j][BOX_SHORT]:
                            have_box = True
                            if door_thick == 18:
                            ...
                            plate1, Num_plies1, Total_plies = self.Package(plies, max_door_height + 50,
                                                                           max_door_width + 50, i)  # i 表示这是第几箱
            ...
            area=0
            if have_board:
                for i in range(len(onesec_door_inform[-1])):
                    if onesec_door_inform[-1][i][5]!=0:
                        area+=onesec_door_inform[-1][i][5]*onesec_door_inform[-1][i][6]
                    #pass
                if (onesec_package_totalplies[-1] <= 2 or sum(onesec_package_Numplies[-1]) <= 3)and area<=500000 :  # （每一包的层数不超过2或最后一包的层数之和不超过3层)且面积不超过0.5平方米，需要进行组合
                    need_combine_doorinform = []
                    for i in range(len(onesec_package_totalplies)):
                        if i == len(
                                onesec_package_totalplies) - 1:  # 循环到最后一包，如果该组件所有的包都是6层，那就得单独打一包了。这种情况目前没有处理，应该从别的包里拆出来几块与该包合到一起。
                            break
                        if onesec_package_totalplies[len(onesec_package_totalplies) - 2 - i] < 6:  # 如果倒数第二包（倒数第i包）的层数小于6层
                            ...
                            onesec_package_Numplies[len(onesec_package_totalplies) - 2 - i][
                                onesec_package_totalplies[len(onesec_package_totalplies) - 2 - i] - 1] = sum(
                                onesec_package_Numplies[-1])
                            # 倒数第二包的索引，               的最后一层块数
                            ...
                            for j in range(len(need_combine_doorinform)):
                                onesec_door_inform[len(onesec_package_totalplies) - 2 - i][
                                    (onesec_package_totalplies[len(onesec_package_totalplies) - 2 - i] - 1) * 10 + j] = \
                                need_combine_doorinform[j]
                ...
                for i in range(len(onesec_package_totalplies)):
                    ...
                    if ERROR_ID == 0:
                        ERROR_ID = self.Post_Package(sec_id, plate1, Num_plies1, Total_plies, Ap_id)

            # 附件的打包 self.accessory_part_information
            if len(self.accessory_part_information) != 0:  # 如果有附件
                sec_id = self.accessory_part_information[0][6]
                ERROR_ID, Ap_id = self.AP_id()  # 下料工位工单编号函数
                if ERROR_ID == 0:
                    ...
                    ERROR_ID = self.Post_Package(sec_id, accessory_info, (0, 0, 0, 0, 0, 0), 0, Ap_id)
    ...
    def Post_Package(self, sec_id, plate1, Num_plies1, Total_plies, Ap_id):
        if (self.Is_Database_Connected()):
            ERROR_ID = self.Post_Package_Select_Workorder(sec_id, plate1, Num_plies1, Total_plies, Ap_id)
            if ERROR_ID == 0:
                return ERROR_ID
    ...
    def Pre_Package_Get_Part_Information(self, sec_id):
        ...
        try:
            ...
            cursor.execute(
                "SELECT  `Door_height`, `Door_width`, `Door_thick`,`Package_state`,`Part_id`,`Door_type`,`Sec_id` ,`Heart_type`,`Element_type_id` FROM `order_part_online` WHERE `Sec_id`='%s' " % sec_id)
            ...
            for i in range(len(part_information1)):  # for循环的作用：检查record列表里是否有高度小于宽度的情况，如果有，交换值,把所有的高度宽度都转为长短边，用于选择箱子尺寸
                ...
                if self.part_information[i][HEIGHT] < self.part_information[i][WIDTH]:  # 以H为大值，W为小值，当W>H时，将H,W转换
            ...
            self.part_information.sort(key=lambda x: (x[HEIGHT], x[WIDTH]), reverse=True)  # 以H为第一优先级，W为第二优先级，进行排序
            for i in range(len(self.part_information)):
                if self.part_information[i][8] == 4 or self.part_information[i][8] == 5 or self.part_information[i][
                    8] == 6:
    ...
    def Pre_Package_Read_Box(self):
        ...
        try:  # 读箱子表单
            cursor = self.db.cursor()
            cursor.execute(
                "SELECT `Box_type`, `Box_long`, `Box_short`, `Plies_18mm`, `Plies_20mm`, `Plies_22mm`, `Plies_25mm` FROM `equipment_package_box` WHERE 1 ORDER BY `Box_long` ,`Box_short` ")
    ...
    def Package(self, plies, height, width, package_num):
        ...
        for j in range(plies):  # 层数，循环一次，调用一次二维排样排一层，至于要排几层由变量plies(层数)控制，这是根据箱子的尺寸及部件的厚度决定的
            edge = 0   # edge==1表示排底层或顶层；edge==0表示非底层、顶层；record[i][7]==0表示部件是实心的
            control = 0
            glass_board_num = [x[7] for x in self.part_information].count(
                '1')  # 这个for循环的作用及目的：每排完一层，判断剩余部件里是否都是玻璃板的，如果是，就允许玻璃板的排顶层或底层，这样有效防止了顶层没有排样的现象，防止丢件。
    ...
    def Layout_2D(self, Record, height, width, edge):
        ...
        while (current_level < total_level):  # 加入这个条件主要是为了，判断每一块新生成的板能否容下未排样的部件
            ...
            for i in range(0, len(record)):
                if record[i][3] == 0:  # Package_state=0
                    # a = record[i][7]
                    if ((edge == 1 and record[i][7] == '0') or edge == 0):  # 实现了玻璃门不能放底层或顶层，edge==1表示排底层或顶层；edge==0表示非底层、顶层；record[i][7]==0表示部件是实心的
                        if ((long_edge >= record[i][0]) and (short_edge >= record[i][1])):  # 芯板能放进去
                            record[i][3] = 1
                            if position >= 10:  # 每层最多放10块,多于10块，溢出
                                spill = True
                                break
                            Result[position] = i + 1  # position表示放芯板在部件信息列表record的id好，如[1,2,9,10,11,0,...]表示列表record中的第1,2,9,10,11号的芯板放好了
                            position += 1
                            ...
                            if ((record[i][0] >= short_edge - record[i][1]) and (Plate[current_level][4] == 0)):  # 新生成第一块待填充芯板是横的，且上一块待填充芯板是横的
                            ...
                            elif ((record[i][0] < short_edge - record[i][1]) and (
                            ...
                            elif ((record[i][1] < long_edge - record[i][0]) and (
                            ...
                            if ((short_edge <= long_edge - record[i][0]) and (
    ...
    def AP_id(self):  # 下料工位工单编号函数
        ...
        try:
            ...
            if (self.Is_Database_Connected()):
                ...
                cursor = self.db.cursor()
                cursor.execute(
                    "SELECT `Ap_id` FROM `work_package_task_list` WHERE 1  ORDER BY `Index` DESC  LIMIT 1")  # 在工单编号数据库里，先按照index降序排列，后取第一行，也就是降序前的最后一行的数据
    ...
    def Post_Package_Select_Workorder(self, sec_id, plate1, Num_plies1, Total_plies, Ap_id):
        '''
        错误代码15120——15130
        :param sec_id:
        :param plate1:
        ...
        for i in range(len(plate1)):
            connect = '&'
            seq = (str(plate1[i][0]), str(plate1[i][1]), str(plate1[i][5]), str(plate1[i][6]), str(plate1[i][4]),
                   str(plate1[i][10]))
        ...
        #             cursor.execute(
        #                 "UPDATE `order_element_online` SET `Package_work_order_ap_id`='%s',`Package_work_order_create_time`='%s' WHERE `Part_id`='%s' " % (
        #                 Ap_id, Contract_C_Time, plate1[i][10]))
        #         cursor.execute("UPDATE `order_part_online` SET `Package_task_list_ap_id`='%s' WHERE `Part_id`='%s' " % (
        #         Ap_id, plate1[i][10]))
        # self.db.commit()
        ...
        try:
            ...
            package_num=0
            cursor.execute("SELECT `Order_id`,`Package_num` FROM `work_package_task_list` WHERE 1  ORDER BY `Index` DESC  LIMIT 1")
            package_num_record = cursor.fetchone()
            if package_num_record is None:
                package_num=1
            else:
                if package_num_record[0]==self.order_id:
                    package_num = package_num_record[1]+1
                else:
                    package_num=1

            print package_infor
            sql = "INSERT INTO `work_package_task_list` (`Create_Time`,`Order_id`,`Package_num`,`Sec_id`,`State`,`Total_plies`,`Total_area`,`Ap_id`, `Long`, `Short`,`Num_plies1`,`Plies1_element_information1`, `Plies1_element_information2`,`Plies1_element_information3`, `Plies1_element_information4`, `Plies1_element_information5`, `Plies1_element_information6`, `Plies1_element_information7`, `Plies1_element_information8`, `Plies1_element_information9`, `Plies1_element_information10`,\
                   ...
                   `Num_plies3`,`Plies3_element_information1`, `Plies3_element_information2`,`Plies3_element_information3`, `Plies3_element_information4`,`Plies3_element_information5`, `Plies3_element_information6`,`Plies3_element_information7`, `Plies3_element_information8`,`Plies3_element_information9`, `Plies3_element_information10`,\
                    ...
                    `Num_plies6`,`Plies6_element_information1`, `Plies6_element_information2`,`Plies6_element_information3`, `Plies6_element_information4`,`Plies6_element_information5`, `Plies6_element_information6`,`Plies6_element_information7`, `Plies6_element_information8`, `Plies6_element_information9`, `Plies6_element_information10`) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % \
                  (Contract_C_Time, self.order_id,package_num, sec_id, '0', Total_plies, total_area, Ap_id, plate1[0][2],
                   plate1[0][3], Num_plies1[0], package_infor[0], package_infor[1], package_infor[2], package_infor[3],
                   package_infor[4], package_infor[5], package_infor[6], package_infor[7], package_infor[8],
                   package_infor[9], Num_plies1[1], package_infor[10], package_infor[11], package_infor[12],
                   package_infor[13], package_infor[14], package_infor[15], package_infor[16], package_infor[17],
                   package_infor[18], package_infor[19],
                   Num_plies1[2], package_infor[20], package_infor[21], package_infor[22], package_infor[23],
                   package_infor[24], package_infor[25], package_infor[26], package_infor[27], package_infor[28],
                   package_infor[29], Num_plies1[3], package_infor[30], package_infor[31], package_infor[32],
                   package_infor[33], package_infor[34], package_infor[35], package_infor[36], package_infor[37],
                   package_infor[38], package_infor[39], Num_plies1[4], package_infor[40], package_infor[41],
                   package_infor[42], package_infor[43], package_infor[44], package_infor[45], package_infor[46],
                   package_infor[47], package_infor[48], package_infor[49], Num_plies1[5], package_infor[50],
                   package_infor[51], package_infor[52], package_infor[53], package_infor[54], package_infor[55],
                   package_infor[56], package_infor[57], package_infor[58], package_infor[59])
    ...
    def Post_Package_Update_Package_State(self):
        ...
        try:
            cursor = self.db.cursor()
            cursor.execute("UPDATE `order_section_online` SET  `Package_state`=5  WHERE `Sec_id`='%s' " % (
            self.sec_id))  # 组件库打包状态置1，表示已经生成打包方案
            cursor.execute("UPDATE `order_part_online` SET  `Package_state`=5  WHERE `Sec_id`='%s' " % (
            self.sec_id))  # 在部件库里，把该组件的打包状态置1
            self.db.commit()
        ...
        try:
            ...
            if len(package_state) == package_state.count(SECTION_PACKAGE_FINISH):
                cursor.execute(
                    "UPDATE `work_production_scheduling` SET  `State`='%s' ,`Schedule_of_package`='%s' WHERE `State`='%s' " % (
                    SCHEDULE_PACKAGE_LAYOUT_FINISH, '今日总包数为' + str(self.all_package_num) + '包', SCHEDULE_LAYOUT_FINISH))
                self.db.commit()
                self.all_package_num = 0
                # self.timer_second.Stop()    #打包结束让时间事件停止。
                cursor.execute("SELECT `Order_id` FROM `order_order_online`  WHERE 1")
                orderid = cursor.fetchall()
                for i in range(len(orderid)):
                    cursor.execute(
                        "SELECT `Index` FROM `work_package_task_list`  WHERE `Order_id`='%s'" % orderid[i][0])
                    index = cursor.fetchall()
                    cursor.execute("UPDATE `order_order_online` SET  `Package_num`='%s'  WHERE `Order_id`='%s' " % (
                    len(index), orderid[i][0]))
