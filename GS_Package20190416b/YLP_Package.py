...
# 20180728a:
# 取消了填写“Package_num_now”字段。
#20180728b:
#是否进行合包改成即看层数和块数也看面积合。
#20180730a:
#在主程序上运行的打包程序加入了时间事件，当到了预定时间后，调用一次打包程序，程序内部的时间事件将所需要打包的所有组件的打包方案生成，然后时间事件停止。
#20180730b:
#把打包程序加入线程。
#20180730c:
#调试的过程中发现了一个问题，如果有一单没下门板只下了附件 程序就会出错，原因是组件编号在打包门板的程序段里获得，如果没有打包门板就没有获得组件号，没有考虑到这种情况。
#20180801a:
#打包程序加入了一个字段，统计本包属于该订单的第几包，该信息用于打印发货单。
        ...
        try:
            ...
            for i in range(len(part_information1)):  # for循环的作用：检查record列表里是否有高度小于宽度的情况，如果有，交换值,把所有的高度宽度都转为长短边，用于选择箱子尺寸
                ...
                if self.part_information[i][HEIGHT] < self.part_information[i][WIDTH]:
                    self.part_information[i][HEIGHT], self.part_information[i][WIDTH] = self.part_information[i][WIDTH], \
                                                                                        self.part_information[i][HEIGHT]
    ...
    def Layout_2D(self, Record, height, width, edge):
        ...
        while (current_level < total_level):  # 加入这个条件主要是为了，判断每一块新生成的板能否容下未排样的部件
            ...
            for i in range(0, len(record)):
                if record[i][3] == 0:
                    a = record[i][7]
                    if ((edge == 1 and record[i][
                        7] == '0') or edge == 0):  # 实现了玻璃门不能放底层或顶层，edge==1表示排底层或顶层；edge==0表示非底层、顶层；record[i][7]==0表示部件是实心的
